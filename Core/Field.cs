...
namespace MainClasses
{
    public class Field
    {
        ...
        internal void AddMove(List<(int, int, int, int)> coordinates)
        {
            if (moves.Count == 0 || moves[AmountMovesOnField].ColorOfMovingPlayer() == Color.White)
            {
                AmountMovesOnField++;
                moves[AmountMovesOnField] = new ChessMove();
                moves[AmountMovesOnField].SetCoordinates(coordinates, Color.White);
            }
            else moves[AmountMovesOnField].SetCoordinates(coordinates, Color.Black);
        }
